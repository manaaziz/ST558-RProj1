...
Then, I created another plot to show the the distribution of the goals scored by dominant hand.  
...
```{r lmeq, include=FALSE}
lm1 <- lm(stat.goals ~ stat.shots + I(stat.shots^2), data = df)
c <- format(unname(coef(lm1)[1]), digits = 2) %>% substring(2)
b <- format(unname(coef(lm1)[2]), digits = 2)
a <- format(unname(coef(lm1)[3]), digits = 2)
eq <- paste0("y = ", a, "x^2 + ", b, "x - ", c)
```

Finally, I wanted to further investigate the relationship between season shots taken and goals scored. I created a scatter plot and overlaid a linear regression line.  
