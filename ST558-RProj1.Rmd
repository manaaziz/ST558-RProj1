...
# Get rid of goalie data in both data sets
VGKrost <- VGKrost %>% filter(position.code != "G")
df.ppl <- df.ppl %>% select(person.id, 1:29)

# Join the data sets by person.id since it is unique
df <- inner_join(VGKrost, df.ppl, by = "person.id")
